...
app.get('/',function(req,res){
    ...
    Pergunta.findAll({ // procure todas as perguntas e ordene elas
        raw: true,
        order: opcao === "antigos" ? [['id', 'ASC']] : [['id', 'DESC']] //DESC = decrescente || ASC = crescente
    }).then(perguntas => {
        res.render('index',{ // renderizar na tela
            perguntas: perguntas
        });
    });
});
...
app.get('/pergunta/:id',(req,res) =>{
    ...
    Pergunta.findOne({ // procure pela pergunta que corresponde ao mesmo id do paramêtro
        where: {id: id}
    }).then(pergunta =>{
        if(pergunta != undefined){ // pergunta foi achada
            ...
            Resposta.findAll({
                ...
                res.render('pergunta',{ // renderizar na tela
                    pergunta: pergunta,
                    resposta: resposta
                });
            });
...
app.get('/categorias/',function(req,res){
    Categoria.findAll({raw: true, order: [ // busque por todas as categorias e ordene elas
        ['id','ASC'] // ASC = crescente
    ]}).then(categorias => {
        res.render('categoria',{ // renderizar na tela
...
app.get('/categorias/:id',(req,res) =>{
    var id = req.params.id;
    Categoria.findOne({ // procure pela categoria que corresponde ao mesmo id do paramêtro
        where: {id: id}
    }).then(categoria =>{
        if(categoria != undefined){ // categoria achada

            Pergunta.findAll({
                where: {categoriaId: categoria.id} //exibir todas as perguntas que tenham o mesmo id da categoria
            }).then(pergunta => {
                res.render('categoriaRes',{ // renderizar na tela
                    categoria: categoria,
                    pergunta: pergunta
                });
            });
